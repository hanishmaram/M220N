...
namespace M220N.Repositories
{
    public class UsersRepository
    {
        ...
        */
        ...
        {
            return await _usersCollection.Find(Builders<User>.Filter.Eq(x => x.Email, email)).FirstOrDefaultAsync(cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                await _usersCollection.WithWriteConcern(new WriteConcern(3)).InsertOneAsync(user);
                //
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                await _sessionsCollection.UpdateOneAsync(Builders<Session>.Filter.Eq(x => x.UserId, storedUser.Id)
                    , Builders<Session>.Update.Set(x => x.UserId, storedUser.Id).Set(x => x.Jwt, storedUser.AuthToken)
